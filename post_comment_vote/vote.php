...
include('../jwt.php');
    ...
    if ($post_id == '' || $cmt_id == ''|| $vote_type == '') {
        array_push($res, ['error'=> true, 'message'=> 'Chưa đủ thông tin']);
        echo json_encode($res);
        die();
    }

    $sql = "INSERT INTO post_comment_vote(post_id,user_id,cmt_id,cmt_parent_id,vote_type) VALUES('$post_id','$user_id','$cmt_id','$cmt_parent_id', '$vote_type')";
    $rl = mysqli_query($conn, $sql);
